...
- P0 缺陷数必须为 0
- 核心 API 冒烟测试（`pytest tests/`）通过率 100%
- 核心页面端到端用例通过率 >= 95%
- 数据任务日运行成功率 >= 95%
...
| 日期 | 说明 |
|------|------|
| 2026-04-18 | 公开热门快照服务端调度为 **约每 3 天**；趋势页 **无手动刷新**，约每 3 天自动拉取；CI 以 `pytest tests/` 替代已删除的 `scripts/contract_test.py` |
